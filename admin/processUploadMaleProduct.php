<?php
    ...
    $sql = "SELECT * FROM male_products WHERE product_name = '$productName'";
    $result = mysqli_query($connection, $sql) or die(mysql_error());
    $row_count = mysqli_num_rows($result);
    if($row_count > 0){
        if($picture_State == "T"){
            
            $sql = "UPDATE male_products SET brand_name = '$brandName', imagetag = 'T', product_description = '$productDescription', product_categories = '$productCategory', product_price = $productPrice WHERE product_name ='$productName'";
            $result = mysqli_query($connection, $sql);
            if($result == true){
                
                $sql = "SELECT product_id FROM male_products WHERE product_name = '$productName'";
            ...
            $sql = "UPDATE male_products SET  product_categories= '$productCategory', brand_name = '$brandName', product_price = '$productPrice', product_description = '$productDescription' WHERE product_name = '$productName'";
            $result = mysqli_query($connection, $sql);
            if($result == true){
                echo 2;
            }else{
        ...
        if($picture_State == "T"){
            ...
            $sql = "INSERT INTO male_products(product_id,product_name,brand_name,product_price, product_categories,product_description,imagetag) 
